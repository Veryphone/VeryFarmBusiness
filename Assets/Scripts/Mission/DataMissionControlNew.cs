...
public class DataMissionControlNew : MonoBehaviour
{
    ...
    //Test mo map san
    public static int MAX_MISSION = 100;
    ...
    void ShowCurrentMission()
    {
        ...
        try
        {
            ...
            for (int i = 0; i < DataCache.dataCurrentMissionCache.Length; i++)
            {
                ...
                Transform missTransform = transform.FindChild(parent).FindChild("MissionItem").FindChild("" + miss);
            ...
            if (VariableSystem.mission > 0)
            {
                ...
                currentPos.x = -transform.FindChild(parent).FindChild("MissionItem").FindChild("" + VariableSystem.mission).localPosition.x;
            }
            if (currentPos.x > 0)
            {
                currentPos.x = 0;
    ...
    void ShowStarMission()
    {
        ...
        for (int i = 0; i < maxmiss; i++)
        {
            ...
            if (missionData[i].Open == 1)
            {
                ...
                try
                {
                    transform.FindChild(parent).FindChild("MissionItem").FindChild("" + missionData[i].Mission).GetComponent<ItemMission>().SetData(missionData[i]);
                }
                catch (Exception e)
                {
    ...
    private void RemoveAllAvatar()
    {
        for (int i = 0; i < MAX_MISSION; i++)
        {
            ...
            try
            {
                transform.FindChild(parent).FindChild("MissionItem").FindChild("" + (i + 1)).GetComponent<ItemMission>().RemoveAllAvatar();
            }
            catch (Exception e)
            {
                Debug.Log("--------Exception----------" + e.Message);
