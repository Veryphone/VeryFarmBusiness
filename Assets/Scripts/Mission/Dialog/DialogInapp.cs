...
public class DialogInapp : DialogAbs
{
    ...
    void Awake()
    {
        ...
        {
            transform.FindChild("Main").FindChild("package" + (i + 1)).FindChild("quatity").GetComponent<UILabel>().text = "" + quatity[i];
            transform.FindChild("Main").FindChild("package" + (i + 1)).FindChild("buy").FindChild("Label").GetComponent<UILabel>().text = "$ " + prices[i];
        }
